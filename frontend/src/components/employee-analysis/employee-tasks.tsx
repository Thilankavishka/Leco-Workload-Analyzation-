...
export const EmployeeTasks = ({ employee, tasks, startDate, endDate, setStartDate, setEndDate }: any) => (
  <Card className="shadow-md">
    <CardHeader>
      ...
      <CardDescription>Tasks assigned to {employee.name} with progress and status</CardDescription>
    </CardHeader>
    <CardContent>
      ...
      {tasks.length > 0 ? (
            ...
            <tbody>
              {tasks.map((task: any) => (
                <tr key={task.id} className="border-b border-gray-200 hover:bg-blue-50 transition-colors">
                  ...
                  <td className="p-3 text-sm text-gray-900">{task.progress === 100 ? "Completed" : "Ongoing"}</td>
                  <td className="p-3 text-sm text-gray-900">{task.startDate}</td>
                  <td className="p-3 text-sm text-gray-900">{task.endDate || "Ongoing"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <p className="text-gray-600 text-center">No tasks found for the selected date range.</p>
      )}
    </CardContent>
  </Card>
);
